...
from utils import divisao_dados_temporais, normalize_serie, desnormalize, split_sequence, reshape_X, forecast_rnn
...
def main(country, gender, init_year):
    ...
    mape_lstm = np.zeros((1, 20))
    n_lags = 2
    # TODO: testando com 10 lags
    ...
    for age, i in zip(ages, range(0,len(ages)+1)):
        ...
        #### TODO: ajuste em 23/06/23 testando lstm na série diferenciada
        serie_norm = normalize_serie(train_i.diff().dropna().values)
        ...
        model, best_hp = get_best_trained_model(X_train_.shape[1], X_train_, y_train, X_val_, y_val)
        ...
        predictions = forecast_rnn(model, X_test_pred, n_lags, 10).ravel()
        ...
        predictions = desnormalize(predictions, train_i.diff().dropna().values).ravel()
        predictions = np.cumsum(np.hstack((train_i.values[0], train_i.diff().dropna(), predictions)))[-10:]
        predictions_ages[:, i] = predictions

        # RMSE
        rmse_lstm[0,i] = MSE(test_i.values, predictions, squared=False)
        mape_lstm[0,i] = MAPE(test_i.values, predictions) * 100 

        print(f"[INFO] LSTM | AGE: {age} | RMSE: {rmse_lstm[0,i]}")
        print(f"[INFO] LSTM | AGE: {age} | MAPE: {mape_lstm[0,i]}")

    # Saving
    predictions_ages = interpolacao_curva(predictions_ages) 
  
    # Exportando os resultados para .csv
    predictions_ages.to_csv(f'./results/univariate/predictions_lstm_{country}_{gender}.csv', index=None, header=True, encoding = 'utf-8')    
...
if __name__ == '__main__':
    ...
    with open(f"./lstm_ages_execution_time.txt", "a") as f:
        f.write(f'[INFO] LSTM | {args.country} | {args.gender} | Execution time: {exec_time}' + '\n')
