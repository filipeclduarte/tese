...
def main(country, gender, init_year):
    # bot_message = ExpBot()
    # bot_message.send_message(f'[INFO] ARIMA_nbeats MIMO - Country: {country} | Gender: {gender} | Initial Year: {init_year}')
    ...
    for age, i in zip(ages, range(0,len(ages)+1)):
        ...
        print(f"[INFO] ARIMA-NBEATS MIMO | AGE: {age} | RMSE: {rmse_hys[0,i]}")
        print(f"[INFO] ARIMA-NBEATS MIMO | AGE: {age} | MAPE: {mape_hys[0,i]}")

    # Saving
    predictions_ages = interpolacao_curva(predictions_ages, ages)

    # Exportando os resultados para .csv
    predictions_ages.to_csv(f'./results/univariate/predictions_arima_nbeats_mimo_{country}_{gender}.csv', index=None, header=True, encoding = 'utf-8')
...
if __name__ == '__main__':
    ...
    with open(f"./arima_nbeats_mimo_ages_execution_time.txt", "a") as f:
        f.write(f'[INFO] ARIMA-NBEATS MIMO | {args.country} | {args.gender} | Execution time: {exec_time}' + '\n')