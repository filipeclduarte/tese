...
def main(country, gender, init_year):
    ...
    for age, i in zip(ages, range(0,len(ages)+1)):
        ...
        print(f"[INFO] NBEATS MIMO | AGE: {age} | RMSE: {rmse_nbeats[0,i]}")
        print(f"[INFO] NBEATS MIMO | AGE: {age} | MAPE: {mape_nbeats[0,i]}")

    # Saving
    predictions_ages = interpolacao_curva(predictions_ages) 
  
    # Exportando os resultados para .csv
    predictions_ages.to_csv(f'./results/univariate/predictions_nbeats_mimo_{country}_{gender}.csv', index=None, header=True, encoding = 'utf-8')
...
if __name__ == '__main__':
    ...
    with open(f"./nbeats_mimo_ages_execution_time.txt", "a") as f:
        f.write(f'[INFO] NBEATS MIMO | {args.country} | {args.gender} | Execution time: {exec_time}' + '\n')
