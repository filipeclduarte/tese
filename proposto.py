...
def main(country, gender, init_year):
    ...
    proposed_ages = predictions_ages_lstm.copy()
    if model_selected == 'mlp':
        proposed_ages = predictions_ages_mlp.copy()
    elif model_selected == 'nbeats':
        proposed_ages = predictions_ages_nbeats.copy()

    # Exportando os resultados para .csv
    proposed_ages.to_csv(f'./results/univariate/predictions_proposed_val_{country}_{gender}.csv', index=None, header=True, encoding = 'utf-8')
    predictions_ages_lstm.to_csv(f'./results/univariate/predictions_arima_lstm_{country}_{gender}.csv', index=None, header=True, encoding = 'utf-8')
    predictions_ages_mlp.to_csv(f'./results/univariate/predictions_arima_mlp_{country}_{gender}.csv', index=None, header=True, encoding = 'utf-8')
    predictions_ages_nbeats.to_csv(f'./results/univariate/predictions_arima_nbeats_{country}_{gender}.csv', index=None, header=True, encoding = 'utf-8')
...
if __name__ == '__main__':
    ...
    # bot_m.send_message(f'[INFO] ARIMA-LSTM | {args.country} | {args.gender} | Execution time: {exec_time}')
