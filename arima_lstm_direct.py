...
def main(country, gender, init_year):
    ...
    rmse_arima_lstm = np.zeros((1,20))
    mape_arima_lstm = np.zeros((1, 20))
    n_lags = 2
    ...
    for age, i in zip(ages, range(0,len(ages)+1)):
        ...
        # RMSE
        rmse_arima_lstm[0,i] = MSE(test_i.values, predictions, squared=False)
        mape_arima_lstm[0,i] = MAPE(test_i.values, predictions) * 100 

        print(f"[INFO] ARIMA-LSTM DIRECT | AGE: {age} | RMSE: {rmse_arima_lstm[0,i]}")
        print(f"[INFO] ARIMA-LSTM DIRECT | AGE: {age} | MAPE: {mape_arima_lstm[0,i]}")

    # Saving
    # predictions_ages = pd.DataFrame(predictions_ages, columns = ages)
    predictions_curve = interpolacao_curva(predictions_ages, ages)
    # Exportando os resultados para .csv
    predictions_curve.to_csv(f'./results/predictions_arima_lstm_direct_{country}_{gender}.csv', index=None, header=True, encoding = 'utf-8')
...
if __name__ == '__main__':
    ...
    with open(f"./arima_lstm_direct_ages_execution_time.txt", "a") as f:
        f.write(f'[INFO] ARIMA-LSTM DIRECT | {args.country} | {args.gender} | Execution time: {exec_time}' + '\n')
