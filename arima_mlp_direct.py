...
def main(country, gender, init_year):
    ...
    predictions_arima = pd.read_csv(f'./results/predictions_arima_{country}_{gender}.csv', sep=',')
    ...
    rmse_arima_mlp = np.zeros((1,20))
    mape_arima_mlp = np.zeros((1, 20))
    n_lags = 2
    ...
    for age, i in zip(ages, range(0,len(ages)+1)):
        ...
        # predictions = desnormalize(predictions, arima_error[-10:]) # inverse normalize with the last 10 observations
        ...
        # RMSE
        rmse_arima_mlp[0,i] = MSE(test_i.values, predictions, squared=False)
        mape_arima_mlp[0,i] = MAPE(test_i.values, predictions) * 100 

        print(f"[INFO] ARIMA-MLP DIRECT | AGE: {age} | RMSE: {rmse_arima_mlp[0,i]}")
        print(f"[INFO] ARIMA-MLP DIRECT | AGE: {age} | MAPE: {mape_arima_mlp[0,i]}")

    # Saving
    # predictions_ages = pd.DataFrame(predictions_ages, columns = ages)
    predictions_ages = interpolacao_curva(predictions_ages, ages)
    # rmse_arima_mlp = pd.DataFrame(rmse_arima_mlp, columns = ages)
    # mape_arima_mlp = pd.DataFrame(mape_arima_mlp, columns = ages)
    
    # Exportando os resultados para .csv
    predictions_ages.to_csv(f'./results/predictions_arima_mlp_direct_{country}_{gender}.csv', index=None, header=True, encoding = 'utf-8')
    # rmse_arima_mlp.to_csv(f'./results/rmse_arima_mlp_direct_{country}_{gender}.csv', index=None, header=True, encoding = 'utf-8')
    # mape_arima_mlp.to_csv(f'./results/mape_arima_mlp_direct_{country}_{gender}.csv', index=None, header=True, encoding = 'utf-8')
...
if __name__ == '__main__':
    ...
    # try:
        # bot_m.send_message(f'[INFO] ARIMA-MLP DIRECT | {args.country} | {args.gender} | Execution time: {exec_time}')
    # except:
        # print(f'[INFO] ARIMA-MLP DIRECT | {args.country} | {args.gender} | Execution time: {exec_time}')
    with open(f"./arima_mlp_direct_ages_execution_time.txt", "a") as f:
        f.write(f'[INFO] ARIMA-MLP DIRECT | {args.country} | {args.gender} | Execution time: {exec_time}' + '\n')
